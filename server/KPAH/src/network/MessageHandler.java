    ...
    public void onMessage(@NonNull ISession session, @NonNull Message msg) {
        try {
            ...
            switch (msg.command) {
                ...
                case CommandMessage.TACH_NGUYEN_LIEU -> {
                    if (player != null && player.getLocation().getZone() != null) {
                        ...
                        if (type == CombineConst.THEM_THUOC_TINH) {
                            ...
                            CombineService.instance.doTachNguyenLieu(player, type, indexItem, idMaterial, iLock);
                        }
                    }
                }
                ...
                case CommandMessage.CMD_NEW_QUEST -> {
                    if (player != null) {
                        services.QuestService.instance.sendQuestInfo(player);
                    }
                }
                ...
                case CommandMessage.REQUEST_REGISTER -> {
                    ...
                    if (username == null || username.trim().isEmpty() || pass == null || pass.trim().isEmpty()) {
                        Service.instance.sendLogOut(session, "Tài khoản hoặc mật khẩu không hợp lệ!");
                        return;
                    }
                    
                    if (username.length() < 3 || pass.length() < 3) {
                        Service.instance.sendLogOut(session, "Tài khoản và mật khẩu phải từ 3 ký tự trở lên!");
                        return;
                    }
                    
                    ResultSetImpl rs = HikariCP.executeQuery("SELECT id FROM users WHERE username = ? LIMIT 1", username);
                    if (rs.next()) {
                        Service.instance.sendLogOut(session, "Tài khoản đã tồn tại!");
                    } else {
                        try (java.sql.Connection conn = HikariCP.getConnection(); java.sql.PreparedStatement ps = conn.prepareStatement("INSERT INTO users (username, password, email, chars, isAdmin, active, luong, xu) VALUES (?, ?, '', '[]', 0, 1, 0, 0)")) {
                            ps.setString(1, username);
                            ps.setString(2, pass);
                            ps.executeUpdate();
                        }
                        Service.instance.sendLogOut(session, "Đăng ký thành công! Vui lòng đăng nhập lại.");
                    }
                }
