...
public class Manager {
    ...
    public static short getLevelAddSkill(byte clazz, int idSkill, int lvSkill) {
        ...
        if (clazz == Const.KIEM_KHACH) {
            return switch (idSkill) {
                case 4 -> (short) (4 < LEVEL_ADD_SKILL.length && lvSkill < LEVEL_ADD_SKILL[4].length ? LEVEL_ADD_SKILL[4][lvSkill] : 3); // Hộ sát tiến / Xuyên giáp passive (base lv 3)
                case 5 -> (short) (5 < LEVEL_ADD_SKILL.length && lvSkill < LEVEL_ADD_SKILL[5].length ? LEVEL_ADD_SKILL[5][lvSkill] : 3); // Dĩ lực đáo công / Phản dame (base lv 3)
    ...
    public static void init() {
        ...
        try {
            ...
            // </editor-fold>
            Logger.log(Logger.BLUE, "Time Load : " + (System.currentTimeMillis() - startTime) + " m/s\n");
        } catch (Exception e) {
