...
## [2026-09-18 09:40] — Task #118: Tối Ưu Giảm Sát Thương Quái Vật Toàn Diện (Cân Bằng Hoàn Hảo Với Hồi Chiêu Bình Máu 10 Giây)

**Yêu cầu:**
1. Khắc phục tình trạng quái vật đánh người chơi quá đau, không tương thích với cơ chế hồi chiêu 10 giây của bình máu:
   - **Giảm mạnh hệ số sát thương cơ bản của quái (`baseAtk`):**
     - Sát thương tối thiểu: Giảm từ `Math.max(20, mobLv * 13 + 10)` xuống `Math.max(8, (int) (mobLv * 6.5 + 5))`.
     - Sát thương tối đa: Giảm từ `Math.max(30, mobLv * 17 + 20)` xuống `Math.max(14, (int) (mobLv * 8.5 + 10))`.
     - Nhờ đó, lượng sát thương quái gây ra chỉ chiếm khoảng **4% - 8% HP tối đa** của người chơi cùng cấp có trang bị cơ bản, giúp người chơi chịu được 8 - 12 đòn cắn, hoàn toàn đồng bộ với nhịp bơm máu 10s.
   - **Loại bỏ hoàn toàn Bạo Kích cho quái thường:** Quái thường không còn tỉ lệ bạo kích gây đột tử bất ngờ.
   - **Giảm chỉ số sát thương quái Tinh Anh (Elite Mob):**
     - Hệ số công giảm từ `x1.8` xuống `x1.35`.
     - Cuồng nộ khi dưới 50% HP giảm từ `x1.25` xuống `x1.15`.
     - Tỉ lệ bạo kích của Tinh Anh giảm từ 25% (x1.5) xuống 15% (x1.3).
   - **Tối ưu sát thương cào xước tối thiểu (`minScratch`):**
     - Quái thường giảm xuống `Math.max(2, (int) (mobLv * 0.6 + 1))`.
     - Quái tinh anh giảm xuống `Math.max(10, (int) (mobLv * 1.2 + 5))`.
     - Người chơi đầu tư giáp thủ cao sẽ nhận sát thương gãi ngứa rất nhỏ, đúng chất RPG cày đồ.
   - **Trả lượng máu quái thường về chuẩn gốc:** Bỏ cơ chế nhân 1.2x HP cho quái thường trong `Monster.getMaxHp()` để tránh kéo dài thời gian giao tranh làm hao hụt lượng HP tích lũy.

**Files thay đổi:**
- `server/KPAH/src/map/Monster.java` — Tối ưu giảm hệ số sát thương cơ bản, bỏ bạo kích quái thường, giảm bonus tinh anh và minScratch, trả HP quái thường về chuẩn gốc.

**Kết quả:** ✅ Thành công
- Server Java biên dịch thành công 100% (`KPAH.jar`).
- Đã sao lưu backup `Monster.java` trong `_backup/` trước khi sửa đổi.

## [2026-09-18 09:48] — Task #119: Khôi Phục Sức Mạnh Tối Đa Cho Quái Tinh Anh (Elite Mob) & Bổ Sung Rơi Tinh Anh Đan (ID 107)

**Yêu cầu:**
1. **Giữ nguyên mức sát thương đã giảm cho quái thường**, nhưng **KHÔNG ĐƯỢC GIẢM quái tinh anh** (Elite Mob) — quái tinh anh phải thật mạnh để tạo thử thách xứng tầm và tránh lạm phát các phần thưởng giá trị (Rương tinh anh, nguyên liệu, đan dược, ngọc rèn).
2. **Khôi phục toàn bộ sức mạnh cho Quái Tinh Anh (`Monster.java`):**
   - Tăng hệ số sát thương cơ bản `baseAtk` của quái tinh anh trở lại `x1.85`.
   - Trạng thái Cuồng Nộ (Frenzy) khi HP < 50%: Tăng thêm **25% sát thương** (`x1.25`).
   - Tỉ lệ Bạo Kích của Quái Tinh Anh: Khôi phục **25% cơ hội bạo kích x1.5 sát thương**.
   - Sát thương cào xước tối thiểu (`minScratch`) thấu giáp: Nâng lên `Math.max(25, (int) (mobLv * 3.5 + 15))` (chống người chơi full giáp tank không mất máu).
   - Cơ chế chênh lệch cấp độ: Quái tinh anh không bị giảm sát thương quá nhiều, tối đa chỉ giảm **25%** (khác với quái thường giảm đến 80%) và luôn đảm bảo không thấp hơn `minScratch`.
3. **Bổ sung cơ chế rơi Tinh Anh Đan (Potion ID: 107):**
   - Quái Tinh Anh khi bị tiêu diệt (`Monster.getItemDrop`) rơi trực tiếp **Tinh Anh Đan (ID: 107)** xuống đất với số lượng 1 - 3 viên tùy theo cấp quái.
   - Thêm phần thưởng **Tinh Anh Đan (ID: 107)** (1 - 2 viên tùy bậc) khi người chơi mở Rương Tinh Anh (`openEliteChest` trong `UseItemService.java`).

**Files thay đổi:**
- `server/KPAH/src/map/Monster.java` — Khôi phục sát thương uy lực cho Quái Tinh Anh (baseAtk x1.85, Frenzy x1.25, 25% crit x1.5, minScratch cao, chênh cấp chỉ giảm max 25%), bổ sung rơi Tinh Anh Đan (ID: 107) trực tiếp khi hạ gục.
- `server/KPAH/src/services/UseItemService.java` — Bổ sung phần thưởng Tinh Anh Đan (ID: 107) khi mở Rương Tinh Anh các bậc.

**Kết quả:** ✅ Thành công
- Quái thường giữ nguyên sát thương nhẹ nhàng, quái tinh anh cực kỳ mạnh mẽ và nguy hiểm.
- Tinh Anh Đan (107) rơi đầy đủ từ quái tinh anh và rương tinh anh.
- Server Java biên dịch thành công 100% (`KPAH.jar`).
- Sao lưu backup `Monster.java` và `UseItemService.java` trong `_backup/` đầy đủ.
